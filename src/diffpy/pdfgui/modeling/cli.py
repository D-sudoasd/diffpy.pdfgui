"""Command-line interface for unified atomic PDF modeling backends."""
...
def _print_table(headers: tuple[str, ...], rows: list[tuple[str, ...]]) -> None:
    ...
    for row in rows:
        clipped = tuple(value if len(value) <= widths[index] else value[: widths[index] - 1] + "…" for index, value in enumerate(row))
