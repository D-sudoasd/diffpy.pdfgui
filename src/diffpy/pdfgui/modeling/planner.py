"""Deterministic workflow planning across small-box, complex, and RMC backends."""
...
checks. Keep numerical observations separate from structural hypotheses. Do not claim that a phase, defect, atom pair,
or disorder mechanism is established without explicit model or experimental support. Do not recommend an unavailable
backend without also giving the installation or external-executable requirement. Respond in {safe_language}.
...
def _steps(backend_id: str, request: ModelingRequest) -> tuple[str, ...]:
    if backend_id in {"pdfgui", "pdffit2"}:
        return (
            "Load the structure and PDF data, then verify scattering type, Qmax, r range, and uncertainty columns.",
            "Refine scale and instrumental terms before lattice, displacement, occupancy, or finite-size parameters.",
            "Inspect Rw, residual autocorrelation, parameter bounds, and physically implausible parameter movement.",
